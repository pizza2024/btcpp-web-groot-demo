...
- Name: loop-concurrency.spec.ts >> 【并发删除场景测试】 >> CD-002-同时删除父子节点: 删除父节点及其子节点应全部删除
- Location: tests/loop-concurrency.spec.ts:224:3
...
Call log:
  - waiting for locator('.react-flow__node').nth(2)
    - locator resolved to <div tabindex="0" role="group" data-id="n_9278o79" aria-roledescription="node" data-testid="rf__node-n_9278o79" aria-describedby="react-flow__node-desc-1" class="react-flow__node react-flow__node-btNode nopan selectable draggable">…</div>
  ...
  - retrying click action
    ...
    - scrolling into view if needed
    - done scrolling
    - <span>🌳</span> from <div tabindex="0" role="group" data-id="n_om81hag" aria-roledescription="node" data-testid="rf__node-n_om81hag" aria-describedby="react-flow__node-desc-1" class="react-flow__node react-flow__node-btNode nopan selectable draggable">…</div> subtree intercepts pointer events
  - retrying click action
    - waiting 20ms
      ...
      - application [ref=e240]:
        - generic [ref=e242]:
          - generic:
            - generic:
              - img:
                - group "Edge from n_giqdujb to n_zfs613n" [ref=e243] [cursor=pointer]:
                  - generic [ref=e248]: ×
              - img:
                - group "Edge from n_giqdujb to n_9278o79" [ref=e249] [cursor=pointer]:
                  - generic [ref=e252]: ×
              - img:
                - group "Edge from n_9278o79 to n_axfe2ru" [ref=e253] [cursor=pointer]:
                  - generic [ref=e258]: ×
              - img:
                - group "Edge from n_9278o79 to n_fb3i11u" [ref=e259] [cursor=pointer]:
                  - generic [ref=e264]: ×
              - img:
                - group "Edge from n_giqdujb to n_om81hag" [ref=e265] [cursor=pointer]:
                  - generic [ref=e270]: ×
            - generic:
              - group [ref=e271]:
...
> 239 |     await allNodes.nth(2).click();
  ...
  305 | 
  306 |     const nodesAfter = await getNodeCount(page);
  307 |     // Should have deleted at least 1 node
  308 |     expect(nodesAfter).toBeLessThan(nodesBefore);
  309 |     expect(await page.locator('.react-flow').isVisible()).toBe(true);
  310 |     expect(await getEdgeCount(page)).toBeLessThanOrEqual(await getEdgeCount(page) + 1);
  311 |   });
  312 | 
  313 |   // ── CD-005: Delete parent with children (no cascade) ────────────────────
  314 | 
  315 |   test('CD-005-删除带子节点的父节点: 当前实现不级联删除子节点(行为记录)', async ({ page }) => {
  316 |     await page.getByRole('button', { name: '📂 Sample' }).click();
  317 |     await page.waitForTimeout(1000);
  318 | 
  319 |     const nodesBefore = await getNodeCount(page);
  320 |     const edgesBefore = await getEdgeCount(page);
  321 | 
  322 |     // Find Sequence node (likely parent)
  323 |     const seqNode = page.locator('.react-flow__node', { hasText: 'Sequence' }).first();
  324 |     const seqVisible = await seqNode.isVisible().catch(() => false);
  325 | 
  326 |     if (seqVisible) {
  327 |       // Deselect first
  328 |       await page.locator('.react-flow__pane').click({ position: { x: 10, y: 10 } });
  329 |       await page.waitForTimeout(200);
  330 | 
  331 |       await seqNode.click();
  332 |       await page.waitForTimeout(100);
  333 |       await page.keyboard.press('Delete');
  334 |       await page.waitForTimeout(600);
  335 | 
  336 |       const nodesAfter = await getNodeCount(page);
  337 |       const edgesAfter = await getEdgeCount(page);
  338 | 
  339 |       // Sequence node should be deleted
```